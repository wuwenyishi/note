...
> 原文：https://pdai.tech/md/java/thread/java-thread-x-juc-executor-FutureTask.html
...
public class FutureTaskTest {
    ...
    public static Integer fdsg() throws Exception {
        ...
        FutureTask<Integer> futureTask = new FutureTask<>(testDemo);
       ...
       TestDemo3 testDemo3 = new TestDemo3();
        FutureTask<Integer> futureTask3 = new FutureTask<>(testDemo3);
        taskExe.submit(futureTask3);
        taskExe.submit(futureTask3);
        ...
        int i = 0;
        for (FutureTask<Integer> futureTasks : taskList) {
            //获取结果，如果任务没有执行完成，get会阻塞
            Integer integer = futureTasks.get();
            i += integer;
        }
        //关闭线程池
        taskExe.shutdown();
...
```java
public class TestDemo implements Callable<Integer> {
    @Override
    public Integer call() throws Exception {
        Thread.sleep(2000);
        return 21;
...
```java
public class TestDemo3 implements Callable<Integer> {
    @Override
    public Integer call() {
        return 24;
    }
...
- 本文主要参考了 https://www.cnblogs.com/linghu-java/p/8991824.html 
